...
def main():
    ...
    parser.add_argument("--prune_idx", default=None, type=int, nargs='+', help="layer index to prune, eg., 1 2 3 10")
    ...
    if args.prune_idx:
        rml = args.prune_idx
        args.num_to_prune = len(rml)
        rml.sort(reverse=True)
        print(rml)
    
        model.model.layers = nn.ModuleList(
            [layer for i, layer in enumerate(model.model.layers) if i not in rml])
        print(f"pruning {args.prune_idx} layers with index {rml}")
        logger.info(f"pruning {args.prune_idx} layers with index {rml}")
    else:
        args.num_to_prune = 0
        print("Test Base model without pruning !")
